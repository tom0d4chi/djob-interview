...
export default function MovieCard({ movie, deleteMovie }: MovieCardProps){
    ...
    return(
        <>
            <div className = {`${isHovered ? 'blur-lg' : 'blur-none'} aspect-[2/3] -z-20 rounded-lg scale-95 transition-all duration-200 absolute inset-0`} style={{backgroundImage: `url(${posterUrl})`}} />
            ...
            <div
                className="absolute inset-0 rounded-lg px-2 py-3 flex flex-col justify-between"
                style={{
                background: `linear-gradient(rgba(0,0,0,0.1) 35%, black 95%)`,
                }}
            > 
            <button className= {`${isHovered ? 'visible opacity-100 text-white' : 'invisible opacity-0'} transition-all`} onClick={() => deleteMovie(movie.id)}>X</button>
